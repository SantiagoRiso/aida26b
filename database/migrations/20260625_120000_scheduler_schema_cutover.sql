-- Initial scheduler schema: auth and scheduler domain tables.
-- Runs in a transaction. No seed/demo rows here.
...
CREATE TABLE clients (
    ...
    -- Every client has a login (no staff-only clients in this deliverable).
...
CREATE TABLE professionals (
    ...
    created_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT professionals_soft_delete_consistency
        CHECK ((deleted_at IS NULL AND deleted_by_user_id IS NULL) OR deleted_at IS NOT NULL)
);
...
CREATE TABLE client_professional_services (
    ...
    created_at      TIMESTAMPTZ   NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ   NOT NULL DEFAULT now(),
    CONSTRAINT client_professional_services_unique UNIQUE (client_id, professional_id, service_id)
);
CREATE INDEX idx_cps_client_id       ON client_professional_services(client_id);
CREATE INDEX idx_cps_professional_id ON client_professional_services(professional_id);
...
CREATE TABLE schedules (
    id              BIGINT      GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    professional_id BIGINT      UNIQUE REFERENCES professionals(id) ON DELETE CASCADE,
    ...
    CONSTRAINT schedules_one_owner CHECK (
        (professional_id IS NOT NULL AND resource_id IS NULL) OR
        (professional_id IS NULL AND resource_id IS NOT NULL))
);
...
CREATE INDEX idx_schedule_exceptions_professional_id ON schedule_exceptions(professional_id);
...
CREATE TABLE calendar_grants (
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT calendar_grants_unique UNIQUE (professional_id, grantee_user_id)
);
CREATE INDEX idx_calendar_grants_professional_id ON calendar_grants(professional_id);
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'aida26_user') THEN
    ...
    EXECUTE 'GRANT SELECT, INSERT, UPDATE ON clients                      TO aida26_user';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE ON professionals                TO aida26_user';
    ...
    -- Protected: writes go through workflow endpoints in later phases.
