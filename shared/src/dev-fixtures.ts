...
export const DEMO_ACCOUNTS = {
  adminUser:             { username: 'demo_admin',          role: 'Admin' },
  professionalUser:      { username: 'demo_pro',            role: 'Professional' },
  receptionistWithGrant: { username: 'demo_recep',          role: 'Receptionist' },
  client:                { username: 'demo_client',         role: 'Client' },
  clientOverdue:         { username: 'demo_client_overdue', role: 'Client' },
  // The ONLY seeded must_change_password account — consumed by forced-password-change.spec.ts only.
  forcedResetUser:       { username: 'demo_reset',          role: 'Professional' },
} as const satisfies Record<string, { username: string; role: Role }>;
