...
// Server enforces per-professional/grant authorization; the picker already restricts a
// Professional to self, so any selected professional is fair game to edit here.
const editable = computed(() => professionalId.value != null);
...
async function loadBlocks(id: number | null) {
  blocks.value = [];
  selectedBlockId.value = null;
  if (id == null) return;
  const res = await listRows('schedule_blocks', { filters: { professional_user_id: String(id) }, limit: 500 });
  if (res.ok) {
...
watch(professionalId, (id) => void loadBlocks(id));

// schedule_blocks columns are typed non-nullable in TableRecordMap (the SSOT type map doesn't
// encode per-column nullability), but resource_id IS nullable at the DB/validator level — this
// write body intentionally sets it null for a professional-owned block, matching GenericForm's cast.
function writeBody(professionalUserId: string, times: WeekdayTimes): Partial<TableRecordMap['schedule_blocks']> {
...
async function onSelect(arg: DateSelectArg) {
  ...
  cal.unselect();
  const profId = professionalId.value;
  ...
  const res = await createRow('schedule_blocks', writeBody(String(profId), decision.body));
  if (res.ok) {
...
async function saveTimes(times: { startTime: string; endTime: string }): Promise<boolean> {
  ...
  const profId = professionalId.value;
  if (block == null || profId == null) return false;
  ...
  const res = await updateRow('schedule_blocks', block.id, writeBody(String(profId), decision.body));
  if (res.ok) {
...
async function applyMove(id: string, startStr: string, endStr: string, revert: () => void) {
  ...
  const res = await updateRow('schedule_blocks', id, writeBody(String(profId), decision.body));
  if (res.ok) {
    const updated = toTemplateBlock(res.data);
...
<template>
  <section ref="sectionRef" class="flex flex-col gap-4">
    ...
    <p
      ...
      class="rounded-lg border border-dashed border-border bg-card px-4 py-10 text-center text-sm text-neutral"
    >
      {{ t('schedule.noProfessional') }}
    </p>
    <template v-else>
