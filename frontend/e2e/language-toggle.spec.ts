...
test.describe('Language toggle', () => {
  ...
  test('SC3: clicking EN toggles nav label from "Calendario" to "Calendar" and back', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Calendario', exact: true })).not.toBeVisible();

    await page.getByTestId('lang-es').click();
    await expect(page.getByRole('link', { name: 'Calendario', exact: true })).toBeVisible({ timeout: 5_000 });
  ...
  test('SC3: EN label also changes page heading from Spanish to English', async ({ page }) => {
    ...
    await expect(heading).toContainText('Configuración');

    await page.getByTestId('lang-en').click();
    await expect(heading).toContainText('Settings', { timeout: 5_000 });

    await page.getByTestId('lang-es').click();
    await expect(heading).toContainText('Configuración', { timeout: 5_000 });
  });
});
