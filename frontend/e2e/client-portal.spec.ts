 ...
 */
test.describe('Client portal — own appointments, balance, and settings', () => {
  ...
  test('client changes interface language via Configuración', async ({ page }) => {
    await login(page, DEMO_ACCOUNTS.client.username, DEMO_ACCOUNTS.client.password);
    await page.getByRole('link', { name: es.nav.settings }).click();

    await expect(page.getByRole('heading', { name: es.nav.settings })).toBeVisible({ timeout: 10_000 });
    await expect(page.getByTestId('language-toggle')).toBeVisible();

    await page.getByTestId('lang-en').click();
    await expect(page.getByRole('link', { name: 'Settings', exact: true })).toBeVisible({ timeout: 5_000 });
    await expect(page.getByRole('heading', { name: 'Settings' })).toBeVisible();

    await page.getByTestId('lang-es').click();
    await expect(page.getByRole('link', { name: es.nav.settings, exact: true })).toBeVisible({ timeout: 5_000 });
    await expect(page.getByRole('heading', { name: es.nav.settings })).toBeVisible();
  });
});
