...
test.describe('Calendar drag-reschedule — conflict flow', () => {
  // Bounded to a handful of clicks — the seeded week is always within a few weeks of "today".
  async function navigateToSeededWeek(page: import('@playwright/test').Page) {
    await page.getByRole('link', { name: 'Calendario' }).click();
    await expect(page.locator('.fc')).toBeVisible();

    for (let i = 0; i < 6; i++) {
      const target = page.locator('[data-appt-state]', { hasText: 'esta semana' });
  ...
  test('calendar renders the seeded scheduled appointments with data-appt-state', async ({ page }) => {
    ...
    await expect(page.locator('[data-appt-state="scheduled"]', { hasText: 'esta semana' }).first())
   ...
   * shared "esta semana" seed data used above.
   */
