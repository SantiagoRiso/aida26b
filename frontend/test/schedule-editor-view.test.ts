...
// tests drive the view's exposed handlers directly, so stub the dialogs' DOM out.
const mountOpts = () => ({ global: { plugins: [makeI18n()], stubs: { BlockEditorModal: true, ConfirmDialog: true } } });
...
async function mountWithProfessionalSelected() {
  const wrapper = mount(ScheduleEditorView, mountOpts());
  await flushPromises();
  // With several professionals the picker is a dropdown; with a single one it auto-selects and
  // renders a label (no dropdown), so only drive the select when it exists.
...
describe('ScheduleEditorView', () => {
  ...
  it('rejects an overlapping selection without calling createRow', async () => {
    const wrapper = await mountWithProfessionalSelected();
  ...
  it('reverts a move that would overlap another block and does not call updateRow', async () => {
    vi.mocked(listRows).mockImplementation(async (table: string) => {
      if (table === 'professionals') {
        return { ok: true, data: [{ id: '1', display_name: 'Dr. Ana' }] };
      }
      if (table === 'schedule_blocks') {
        return { ok: true, data: [
          existingBlock,
  ...
  it('rejects a modal time edit that overlaps another block without calling updateRow', async () => {
    vi.mocked(listRows).mockImplementation(async (table: string) => {
      if (table === 'professionals') return { ok: true, data: [{ id: '1', display_name: 'Dr. Ana' }] };
      if (table === 'schedule_blocks') {
        return { ok: true, data: [
